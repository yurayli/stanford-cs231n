...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        a1, l1_cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, l2_fc_cache = affine_forward(a1, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, grads['W2'], grads['b2'] = affine_backward(dx, l2_fc_cache)
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dx, l1_cache)

        loss += 0.5 * self.reg * \
            sum([np.square(self.params['W1']).sum(), np.square(self.params['W2']).sum()])
    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm:
            for i in range(1, self.num_layers):
                a, cache = affine_bn_relu_forward(a, self.params['W%d'%i], self.params['b%d'%i],
            ...
            for i in range(1, self.num_layers):
                ...
                caches.append(cache)
        scores, cache = affine_forward(a, self.params['W%d'%(self.num_layers)],
        ...
        if self.use_batchnorm:
            ...
            for i in range(self.num_layers-1, 0, -1):
                dx, grads['W%d'%i], grads['b%d'%i] = affine_relu_backward(dx, caches[i-1])

        loss += 0.5 * self.reg * \
            sum([np.square(self.params['W%d'%i]).sum() for i in range(1, self.num_layers+1)])
        for i in range(1, self.num_layers+1):
